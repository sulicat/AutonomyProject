...
void print_cost_map( cost_map_generator::CostMap cost_map ){
    ...
    char* image = "global_cost_map.bmp";
    stbi_write_bmp(image, cost_map.cell_num_y, cost_map.cell_num_x, 1, cost_map.cells.data());
}
...
void generate_cost_map_global( cost_map_generator::CostMap& cost_map, cost_map_generator::ObstacleList& obst_list ){
    ...
    cost_map.cell_num_x = (int)(temp_w/cell_size + 0.5);
    cost_map.cell_num_y = (int)(temp_h/cell_size + 0.5);
	...
	for( int c = 0; c < cost_map.cell_num_x; c++ ){
	    ...
	    float _x = c * cost_map.cell_dimension;
	    float _y = r * cost_map.cell_dimension;
